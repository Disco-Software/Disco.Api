...
class RegistrationBloc extends Bloc<RegistrationPageEvent,RegistrationPageState>{
  RegistrationBloc(initialState) : super(initialState){
    on<RegistrationEvent>((event, emit) => emit.forEach<RegistrationPageState>(_handleRegistration(event), onData: (state) => state));
  }
  final _authApi = AuthApi();

  Stream<RegistrationPageState> _handleRegistration(RegistrationEvent event) async*{
    yield RegistratingState();
    final response = await _authApi.registration(RegisterRequestModel(userName: event.userName, email: event.email, password: event.password, confirmPassword: event.confirmPassword));
    if(response?.user != null && response?.verificationResult != null) {
      ...
      switch(errorText){
        case "User name is requared" : yield RegistrationErrorState(userName: "User name is requared");
        break;
        case "Email is required": yield RegistrationErrorState(email: errorText);
        break;
        case "Email must be a valid email address" : RegistrationErrorState(email: errorText);
        break;
        case "Password is required" : RegistrationErrorState(password: errorText);
        break;
        default : "";
